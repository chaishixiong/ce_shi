...
class TaoBaoQiYeShop(object):
    ...
    def query_sql_data(self, i):
        jd_cp_sql = 'SELECT shop_id,company,company_address,province,city,county FROM `taobao_qiye_shopinfo_{}_cp`'.format(i)
        jd_cp_data = self.session_228_jd.query_tuple_data(jd_cp_sql)
        jd_hangzhou_sql = 'SELECT * FROM `taobao_qiye_shopinfo_{}_hangzhou`'.format(i)
    ...
    def insertion_sql_data(self, i):
        insertion_sql = "insert into taobao_qiye_shopinfo_{} (shop_id,seller_id,bc_type,main_sale,company,sales_count,sales_money,address,seller_name,seller_lv,shop_hpl,shop_name,weitao_id,shop_fans_num,open_shop_date,goods_num,new_goods_num,guanzhu_num,shop_money,shop_iocn,gold_shop,shop_type,taobao_shop_type2,taobao_shop_type3,open_year,company_change,company_address,province,city,county,street,last_sales_money,vix,shop_url,xinyong,describe_rate,service_rate,logistics_rate,township) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)".format(i)
    ...
    def data_updata(self, i, p_c_c_list, hangzhou_data_list):
        ...
        for hangzhou_data in hangzhou_data_list:
            ...
            if p_c_c_data != None:
                ...
                hangzhou_data_l[26] = p_c_c_data[2]
                hangzhou_data_l[27] = p_c_c_data[3]
                hangzhou_data_l[28] = p_c_c_data[4]
                hangzhou_data_l[29] = p_c_c_data[5]
