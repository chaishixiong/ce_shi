...
class MTWenZhouData(object):
    ...
    def insertion_data(self, data_list):
        insertion_sql = "insert into meituan_wenzhou_new (shop_id, shop_name, avg_score, all_comment_num, address, avg_price, category, province, city, county, street) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
def run_mt_wenzhou_spider():
    ...
    with open('data_data.txt', 'r', encoding='utf-8') as rf:
        for i in rf:
            data_list = i.strip('\n').split(',')
            ...
            print(data_list)
            run_mt_wz.insertion_data(data_list)
