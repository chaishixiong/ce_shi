...
class JdShop(object):
    ...
    def query_sql_data(self, i):
        jd_cp_sql = 'SELECT shop_id,company,company_address,province,city,county FROM `jd_shopinfo_{}_cp`'.format(i)
        jd_cp_data = self.session_228_jd.query_tuple_data(jd_cp_sql)
        jd_hangzhou_sql = 'SELECT * FROM `jd_shopinfo_{}_hangzhou`'.format(i)
    ...
    def insertion_sql_data(self, i):
        insertion_sql = "insert into jd_shopinfo_{} (shop_id,venderid,shop_name,fangs_num,goods_num,new_goods_num,hot_goods_num,cuxiao_goods_num,shop_info,open_shop_date,pingjia_num,fuwu_num,wuliu_num,company,shop_address,shop_pp,sales_money,main_sale,cname2,cname3,bname,data_month,company_address,province,city,county,street,sales_count,last_sales_money,vix,shop_url,main_sale_count,main_sale_all,main_sale_top5,township) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)".format(i)
    ...
    def data_updata(self, i, p_c_c_list, hangzhou_data_list):
        ...
        for hangzhou_data in hangzhou_data_list:
            ...
            if p_c_c_data != None:
                hangzhou_data_l[13] = p_c_c_data[1]
                hangzhou_data_l[22] = p_c_c_data[2]
                hangzhou_data_l[23] = p_c_c_data[3]
                hangzhou_data_l[24] = p_c_c_data[4]
                hangzhou_data_l[25] = p_c_c_data[5]
