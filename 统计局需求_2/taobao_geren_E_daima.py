...
from 统计局需求_2.sql_pool.dbpool import tb_pool, shop_228_pool
...
class GerenAddress(object):
    ...
    def geren_address(self, i, shop_list_list):
        ...
        for shop_list in shop_list_list:
            sql = "select * from `taobao_goodsmobile_{}` where seller_id in {} limit 1000".format(i, tuple(shop_list))
    ...
    def insertion_data(self, i):
        ...
        insertion_sql = "insert into taobao_goodsmobile_{}_E_ah_all (update_date, goods_id, sales_month, sales_month1, comment_count, inventory, collect_num, address, cid, bid, bc_type, goods_name, seller_id, shop_id, advertising, discount_price, earnest, earnest_desc, price_name, price, ship_cost, assurance, promotion, promotion_desc, rewards, image_url, goods_info, E_daima) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)".format(i)
    ...
    def taobao_228_geren_shop_id(self, i, shop_id_list):
        sql = "SELECT seller_id FROM `taobao_shopinfo_{}` WHERE `company` = '' AND `province` = '上海'".format(i)
    ...
    def taobao_228_qiye_shop_id(self, i, shop_id_list):
        sql = "SELECT seller_id FROM `taobao_qiye_shopinfo_{}` WHERE `province` = '上海市'".format(i)  # 上海市 江苏省 安徽省
        all_res = self.session_228.query_tuple_data(sql)
        for tuple_shopid in all_res:
            shopid = tuple_shopid[0]
            shop_id_list.append(shopid)
