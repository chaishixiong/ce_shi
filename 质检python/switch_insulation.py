...
select * from
(select a.date,ifnull(100*(1-(a.value+b.value)/(c.value+d.value)),0) as value from 
(select left(approval_date,7) as date, count(1) as value from dwd_zj_taskitem where test_basis like '%GB/T 16915.1-2014%' and measured_value != 'NULL' and item_l1 = '绝缘材料的耐非正常热、耐燃和耐电痕化'  and measured_value = '符合' and above_scale_flag = 0 or test_basis like '%GB/T16915.1-2014%' and measured_value != 'NULL' and item_l1 = '绝缘材料的耐非正常热、耐燃和耐电痕化' and measured_value  = '符合' and above_scale_flag = 0  group by left(approval_date,7))a

left join 
(select left(approval_date,7) as date, 3*count(1) as value from dwd_zj_taskitem where test_basis like '%GB/T 16915.1-2014%' and measured_value != 'NULL' and item_l1 = '绝缘材料的耐非正常热、耐燃和耐电痕化'  and measured_value  = '符合' and above_scale_flag = 1 or test_basis like '%GB/T16915.1-2014%' and measured_value != 'NULL' and item_l1 = '绝缘材料的耐非正常热、耐燃和耐电痕化' and measured_value  = '符合' and above_scale_flag = 1  group by left(approval_date,7))b on a.date = b.date

left join (
select left(approval_date,7) as date, count(1) as value from dwd_zj_taskitem  where test_basis like '%GB/T 16915.1-2014%' and measured_value != 'NULL' and item_l1 = '绝缘材料的耐非正常热、耐燃和耐电痕化' and above_scale_flag = 0 or test_basis like '%GB/T16915.1-2014%' and measured_value != 'NULL' and item_l1 = '绝缘材料的耐非正常热、耐燃和耐电痕化' and above_scale_flag = 0 group by left(approval_date,7))c
on a.date = c.date 
left join (
select left(approval_date,7) as date, 3*count(1) as value from dwd_zj_taskitem  where test_basis like '%GB/T 16915.1-2014%' and measured_value != 'NULL' and item_l1 = '绝缘材料的耐非正常热、耐燃和耐电痕化' and above_scale_flag = 1 or test_basis like '%GB/T16915.1-2014%' and measured_value != 'NULL' and item_l1 = '绝缘材料的耐非正常热、耐燃和耐电痕化' and above_scale_flag = 1 group by left(approval_date,7))d
on a.date = d.date 


order by a.date desc limit 19) t order by t.date 
'''
    cursor = db.cursor()
    cursor.execute(sql)
...
def XGBoost(data):
    ...
    data_train1 = data.iloc[0:18]

    data_standard = pd.DataFrame()
    data_standard['date'] = data['date'].iloc[0:19]
    standard_list = []
