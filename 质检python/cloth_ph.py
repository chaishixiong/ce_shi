...
select a.date, avg(a.value) as value from (
select left(approval_date, 7) as date, measured_value+0 as value from dwd_zj_taskitem where (test_basis like '%GB/T 22854-2009%'  or test_basis like '%GB/T 23328-2009%' or test_basis like '%GB/T 31888-2015%') and measured_value != 'NULL' and item_l1 = 'pH值(KCl萃取液)' and above_scale_flag = 0
union all 
select left(approval_date, 7) as date, measured_value+0 as value from dwd_zj_taskitem where (test_basis like '%GB/T 22854-2009%'  or test_basis like '%GB/T 23328-2009%' or test_basis like '%GB/T 31888-2015%') and measured_value != 'NULL' and item_l1 = 'pH值(KCl萃取液)' and above_scale_flag = 1
union all
select left(approval_date, 7) as date, measured_value+0 as value from dwd_zj_taskitem where (test_basis like '%GB/T 22854-2009%'  or test_basis like '%GB/T 23328-2009%' or test_basis like '%GB/T 31888-2015%') and measured_value != 'NULL' and item_l1 = 'pH值(KCl萃取液)' and above_scale_flag = 1
union all
select left(approval_date, 7) as date, measured_value+0 as value from dwd_zj_taskitem where (test_basis like '%GB/T 22854-2009%'  or test_basis like '%GB/T 23328-2009%' or test_basis like '%GB/T 31888-2015%') and measured_value != 'NULL' and item_l1 = 'pH值(KCl萃取液)' and above_scale_flag = 1) a group by a.date order by a.date desc limit 21
            ) a order by a.date
...
def XGBoost(data):
    ...
    data_train1 = data.iloc[0:20]

    data_standard = pd.DataFrame()
    data_standard['date'] = data['date'].iloc[0:21]
    standard_list1 = []
    standard_list2 = []
    for i in range(len(data_standard)):
        standard_list1.append(8.5) #标准值小于8.5
        standard_list2.append(4)  # 标准值大于4
    data_standard['value1'] = standard_list1
    data_standard['value2'] = standard_list2
    ...
    return  item(y_train)\
        ...
        , item(data_standard['value1'].values.tolist()), item(data_standard['value1'].values.tolist())[0], item(data_standard['value2'].values.tolist()), item(data_standard['value2'].values.tolist())[0]
