...
def load_data():
    ...
    sql = '''   
select a.value, a.date from(
 select avg(t.measured_value) as value, t.date from(
select sample_name, item_l1, standard_value, replace(replace(replace(measured_value,'<',''),'＜',''),'NULL','0') as measured_value, left(approval_date,7) as date from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '外底耐磨性能（mm）'  and measured_value not like '%未%' and above_scale_flag =1
union all 
select sample_name, item_l1, standard_value, 0 as measured_value, left(approval_date,7) as date from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '外底耐磨性能（mm）'  and measured_value  like '%未%' and above_scale_flag =1
union all
select sample_name, item_l1, standard_value, replace(replace(replace(measured_value,'<',''),'＜',''),'NULL','0') as measured_value, left(approval_date,7) as date from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '外底耐磨性能（mm）'  and measured_value not like '%未%' and above_scale_flag =1
union all 
select sample_name, item_l1, standard_value, 0 as measured_value, left(approval_date,7) as date from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '外底耐磨性能（mm）'  and measured_value  like '%未%' and above_scale_flag =1
...
select sample_name, item_l1, standard_value, 0 as measured_value, left(approval_date,7) as date from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '外底耐磨性能（mm）'  and measured_value  like '%未%' and above_scale_flag =1
union all
select sample_name, item_l1, standard_value, replace(replace(replace(measured_value,'<',''),'＜',''),'NULL','0') as measured_value, left(approval_date,7) as date from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '外底耐磨性能（mm）'  and measured_value not like '%未%' and above_scale_flag =0
union all 
select sample_name, item_l1, standard_value, 0 as measured_value, left(approval_date,7) as date from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '外底耐磨性能（mm）'  and measured_value  like '%未%' and above_scale_flag =0) t group by t.date order by t.date desc limit 22
            ) a order by a.date
...
def XGBoost(data):
    ...
    return  item(y_train)\
        , item(data_zero1), item(data_zero2)\
        , item(data_test['date'].values.tolist())[-1], item(data_test['value'].values.tolist())[-1], item(data_pred['date'].values.tolist())[-1]\
        , item(data_pred['value'].values.tolist())[-1], item(data_standard['date'].values.tolist())\
        , item(data_standard['value'].values.tolist()), item(data_standard['value'].values.tolist())[0]
