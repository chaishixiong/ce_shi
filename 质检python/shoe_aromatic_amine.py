...
(select  left(approval_date,7) as date, count(1) as value from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '可分解有害芳香胺染料(mg/kg)' and measured_value  like '%未检出%' and above_scale_flag = 0 or test_basis like '%QB/T 1002-2015%' and item_l1 = '可分解有害芳香胺染料(mg/kg)' and measured_value  like '%符合%' and above_scale_flag = 0  group by left(approval_date,7)) b 
left join 
(select  left(approval_date,7) as date, 3*count(1) as value from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '可分解有害芳香胺染料(mg/kg)' and measured_value  like '%未检出%' and above_scale_flag = 1 or test_basis like '%QB/T 1002-2015%' and item_l1 = '可分解有害芳香胺染料(mg/kg)' and measured_value  like '%符合%' and above_scale_flag = 1  group by left(approval_date,7)) a on b.date = a.date
left join (
select left(approval_date,7) as date, 3*count(1) as value from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '可分解有害芳香胺染料(mg/kg)' and measured_value not like '%不%' and measured_value != 'NULL' and above_scale_flag = 1 group by left(approval_date,7) ) c on b.date = c.date
left join (
select left(approval_date,7) as date, count(1) as value from dwd_zj_taskitem where test_basis like '%QB/T 1002-2015%' and item_l1 = '可分解有害芳香胺染料(mg/kg)' and measured_value not like '%不%' and measured_value != 'NULL' and above_scale_flag = 0 group by left(approval_date,7) ) d on b.date = d.date order by date desc limit 14) e order by e.date
'''
    print(3)
    cursor = db.cursor()
    cursor.execute(sql)
...
def XGBoost(data):
    ...
    data_train1 = data.iloc[0:13]

    data_standard = pd.DataFrame()
    data_standard['date'] = data['date'].iloc[0:14]
    standard_list = []
