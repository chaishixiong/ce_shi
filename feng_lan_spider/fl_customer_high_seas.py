...
class FengLanGongHai(object):
    ...
    def process_item(self, rccords, detail_data, contact_data, call_log_data, site_data):
        ...
        #         f.write(i + ',')
        #     f.write('\n')
        # print(len(detail_item))
        #  crm, system_id, company_name, company_email, area, source_id, industry, level, login_email, seller_name, website, company_phone, address, state, products, password, create_time, keep_time, update_time, contact_time, remark
    ...
    def run_feng_lan(self):
        for self.num in range(1, 1258):
            list_url = 'https://api.fomille.com/agent/api/merchant/merchant/open-paging'
            ...
            parmas_list["params"]["states"] = [0]
            response = self.request_post(list_url, parmas_list)
            # rccords_dict = self.rccords_str(response)
            rccords_list = response.get('data').get('records')
            if rccords_list != None:
