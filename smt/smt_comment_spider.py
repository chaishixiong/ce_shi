...
class CommentSpyder(threading.Thread):
    ...
    def crawl_comment_by_page(self,page):
        ...
        try:
            # proxies=self.get_random_ip()
            comment_html=self.session.post(self.posturl,headers=self.headers,data=self.data,timeout=30)
...
def main(productid,owner_memberid,companyid,thread_num=10):
    ...
    #         if (i == (thread_num - 1)):
    #             end_page=total_page
    spyder = CommentSpyder(url,productid,owner_memberid,companyid,result_queue)
    spyder.run()
...
if __name__ == '__main__':
    productid = "1005001865097296"
    owner_memberid = "201766682"
    companyid = "214336312"
